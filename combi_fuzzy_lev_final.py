...
# Now start with the second part of the code
with open("/content/n_bbox/output.txt", "r") as file:
    lines = file.readlines()

# Extract only the text part from each line
text_parts = [line.split(' | ')[0].split(',')[0] for line in lines]
...
while i < len(text_parts) - 1:
    ...
    if similarity_ratio > 70:
        print(f"Removing line: {lines[i + 1]}\nBecause it's {similarity_ratio}% similar to: {lines[i]}\n")
        del lines[i + 1]
        del text_parts[i + 1]
    else:
        i += 1
